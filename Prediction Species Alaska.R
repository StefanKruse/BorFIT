    ...
    for (i in 1:length(unique_values_count)) {
      ...
      if(unique_values_count[i]!=1 & unique_values_count[i]!=2 & tree@header$`Number of point records`>50 & max(tree@data$Z)>1 ){ 
        ...
        if (nrow(widest_segments)>0){        
          ...
          if (is.nan(top_angle)){ top_angle<-0}
          ...
          Abies<-data.frame(PIMA=predicted_labels$PIMA, PIGL=predicted_labels$PIGL, PISI=predicted_labels$PISI)
          Abies<-names(which.max(Abies))
          probably<-0
          if(predicted_classes == "ABLA"){predicted_classes<-Abies
          probably<-predicted_labels[colnames(predicted_labels)==Abies]}
