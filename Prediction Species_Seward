...
for (n in 1:length(test)) {
  result <- tryCatch({  
    ...
    predict_data<-data.frame(Tree = NULL,LAT=NULL, LONG =NULL, Zq999 = NULL,VARI=NULL, NGRDI=NULL, Zq99=NULL,vol_concave= NULL, CV_Z=NULL, CRR=NULL, density_concave =NULL,  vertical_variability=NULL, pointedness =NULL, top_angle =NULL,Z_widest_distance =NULL, relative_height_widest=NULL, widest_distance = NULL, Probability=NULL)
    points <- las@data
                                                            ...
                                                            ifelse(predicted_classes =="POTR",9,
                                                                   ifelse(predicted_classes =="POBA",10,
                                                                          ifelse(predicted_classes =="PISI",11,
                                                                                 ifelse(predicted_classes =="ALNUS",12,
                                                                                        ifelse(predicted_classes =="BENE",4,
                                                                                        ))))))))))) 
          result$Tree<-unique_values_count[i]
          result$Species<- code
          result$Probability<-as.numeric(predicted_labels$PIGL)
        }} else {
