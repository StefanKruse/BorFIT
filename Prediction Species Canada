...
get_crown_attributes <- function(X, Y, Z, export_ashape = FALSE) {
  ...
  # Return metrics and ashape object as a list
  if (export_ashape) {
...
for(n in 5:length(test)) {
  result <- tryCatch({  
    las<-readLAS(test[n])
    if (is.null(las$R)){next}
    predict_data<-data.frame(Tree = NULL, Zq999 = NULL,VARI=NULL, NGRDI=NULL, Zq99=NULL, vol_concave= NULL, CV_Z=NULL, CRR=NULL, density_concave =NULL,  vertical_variability=NULL, pointedness =NULL, top_angle =NULL,Z_widest_distance =NULL, relative_height_widest=NULL, widest_distance = NULL, Probability=NULL)
    points <- las@data
    ...
    for (i in 1:length(unique_values_count)) {
      ...
      if(unique_values_count[i]!=1 & unique_values_count[i]!=2 & tree@header$`Number of point records`>50 & max(tree@data$Z)>1 ){ 
        ...
        if (nrow(widest_segments)>0){        
          ...
          result$VARI <- mean(ifelse((tree$G + tree$R + tree$B) == 0, NA, (tree$G - tree$R) / (tree$G + tree$R + tree$B)), na.rm = TRUE)
          result$NGRDI <- mean(ifelse((tree$G + tree$R) == 0, NA, (tree$G - tree$R) / (tree$G + tree$R)), na.rm = TRUE)
          ...
          result$vertical_variability <- sd(tree$Z)          # vertical variability of points
          ...
          Picea<-data.frame(PIMA=predicted_labels$PIMA, PIGL=predicted_labels$PIGL)
          Picea<-names(which.max(Picea))
          probably<-0
          if(predicted_classes == "PISI"){predicted_classes<-Picea
          probably<-predicted_labels[colnames(predicted_labels)==Picea]}
          if(predicted_classes == "BENE"){predicted_classes<-"BEPA"
          probably<-predicted_labels[colnames(predicted_labels)=="BEPA"]}
          code <- ifelse(predicted_classes == "PIMA", 3,
                         ifelse(predicted_classes == "PIGL", 4,
                                ifelse(predicted_classes =="PICO",5,
                                       ifelse(predicted_classes =="ABLA",6,
                                              ifelse(predicted_classes =="LALA",7,
                                                     ifelse(predicted_classes =="BEPA",8,
                                                            ifelse(predicted_classes =="POTR",9,
                                                                   ifelse(predicted_classes =="POBA",10,
                                                                          ifelse(predicted_classes =="PISI",11,
                                                                                 ifelse(predicted_classes =="ALGL",12,
                                                                                        ifelse(predicted_classes =="BENE",13,
                                                                                        )))))))))))
          
          result$Tree<-unique_values_count[i]
          result$Species<- code
          result$Probability<-as.numeric(max(predicted_labels)+probably)
        }} else {
